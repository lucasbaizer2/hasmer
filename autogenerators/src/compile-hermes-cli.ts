...
import { DefinitionFile, cloneHermesRepository, getDefinitionFiles, git } from './common';
...
function compileV1(definition: DefinitionFile, destRoot: string) {
    ...
    const destDir = `${destRoot}/cli-versions/${definition.Version}/${os.platform()}`;
...
function compileV2(definition: DefinitionFile, destRoot: string) {
    const buildSystem = getPlatformBuildSystem();
    const configureProc = child_process.spawnSync(
        'cmake',
        ['-B', 'build', '-G', buildSystem],
        {
            stdio: 'inherit',
            cwd: 'hermes',
        }
    );
    if (configureProc.error) {
        console.log('cmake configuration returned an error, exiting: ' + configureProc.error);
        return;
    }
    const buildProc = child_process.spawnSync(
        'cmake',
        ['--build', './build'],
        {
            stdio: 'inherit',
    ...
    if (buildProc.error) {
        console.log('cmake build returned an error, exiting: ' + buildProc.error);
        return;
    }
    ...
    fsExtra.copySync('hermes/build/bin', destDir, {
        overwrite: true,
        recursive: true,
    });
}
...
async function main() {
    ...
    for (const definition of definitions) {
        ...
        console.log('Resetting git checkout');
        ...
        let compile: (definition: DefinitionFile, destRoot: string) => void;
        if (definition.Version < 86) {
            compile = compileV1;
        } else {
            compile = compileV2;
        }
        compile(definition, '/ext');
    }
}
